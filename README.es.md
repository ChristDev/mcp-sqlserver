...
- **Guardrails por fuente** — Modo solo-lectura, límite de filas, timeout de query por base de datos
- **Health Checks** — Valida red, autenticación y query al iniciar con mensajes claros
- **Docker Ready** — `network_mode: host` para acceso via VPN/red corporativa
...
Por fuente. Cuando está activo, solo se permiten `SELECT`, `WITH`, `EXPLAIN`, `SHOWPLAN`.
...
Timeout por fuente en segundos. Mensaje de error claro cuando se excede.
...
Docker Container (mcp-sqlserver)
    |  network_mode: host (hereda VPN/red corporativa)
    |
...
| **v2.0** | SSH Tunneling, lazy connection pooling, reintentos de conexión |
